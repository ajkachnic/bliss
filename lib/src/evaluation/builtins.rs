...
use super::{EvalResult, Evaluator, object::{BuiltinFunc, Object}};
...
fn map(args: Vec<Object>, eval: Rc<RefCell<Evaluator>>) -> EvalResult {
  if let Object::Array(array) = args[0].clone() {
    match args[1].clone() {
      Object::Function{body, env, parameters} => {
        let function = Object::Function{body, env, parameters};
        let mut arr = vec![];
        for element in array {
          let res = eval.borrow_mut().eval_function_call(function.clone(), vec![element])?;
          arr.push(res);
        }
        return Ok(Object::Array(arr))
      },
      arg => {
        return Err(format!("Expected function, got {}", arg))
      }
